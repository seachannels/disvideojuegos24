      ...
      private void OnEnable()
{
    if (playerInput == null)
    {
        ...
        playerInput.actions["Move"].performed += ctx => moveDirection = ctx.ReadValue<Vector2>();
        playerInput.actions["Move"].canceled += ctx => moveDirection = Vector2.zero;

         playerInput.actions["Interact"].performed += InteractButtonPressed;
        playerInput.actions["Interact"].canceled += InteractButtonPressed;

        playerInput.actions["Submit"].performed += SubmitPressed;
        playerInput.actions["Submit"].canceled += SubmitPressed;
    }
